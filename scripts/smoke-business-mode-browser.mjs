...
async function run() {
  ...
  try {
    ...
    await waitFor(
      "document.readyState === 'complete' && Boolean(document.querySelector('nav[aria-label=\"Asympta World mode\"]')) && Boolean(document.querySelector('form.asympta-intent-composer'))",
      15_000,
      "Users world and mode switch",
    );
    ...
    const structure = JSON.parse(await evaluate(`JSON.stringify((() => {
      ...
      return {
        ...
        importButtons: [...(world?.querySelectorAll('button') ?? [])]
          .filter((button) => button.textContent?.includes('Import JSON / CSV')).length,
    ...
    // Dispatch both file imports synchronously. Node-side waitFor below is the only
    // asynchronous settling boundary, so Chrome cannot garbage-collect a page Promise.
    await evaluate(`(() => {
      const world = document.querySelector('[data-asympta-business-world="true"]');
    ...
    await waitFor(
      "document.querySelector('[data-asympta-business-world=\"true\"]')?.textContent?.includes('Harbour Bakery') && document.querySelector('[data-asympta-business-world=\"true\"]')?.textContent?.includes('Sourdough loaf')",
      8_000,
      "imported business and product data",
    );

    await evaluate(`(() => {
      const world = document.querySelector('[data-asympta-business-world="true"]');
      const composer = [...world.querySelectorAll('textarea')]
        .find((textarea) => textarea.placeholder?.startsWith('Customer agent:'));
      if (!composer) throw new Error('Business agent composer is unavailable.');
    ...
    await waitFor(
      `(() => {
        const world = document.querySelector('[data-asympta-business-world="true"]');
        const send = [...(world?.querySelectorAll('button') ?? [])]
          .find((button) => button.textContent?.includes('Run communication'));
        return Boolean(send && !send.disabled);
      })()`,
      8_000,
      "Business agent communication button to become enabled",
    );
    ...
    await evaluate(`(() => {
      ...
      const send = [...world.querySelectorAll('button')]
        .find((button) => button.textContent?.includes('Run communication'));
      if (!send || send.disabled) throw new Error('Business agent communication button is unavailable.');
      send.click();
      return true;
    })()`);

    await waitFor(
      "document.querySelector('[data-asympta-business-world=\"true\"]')?.textContent?.includes('HKD 48') && document.querySelector('[data-asympta-business-world=\"true\"]')?.textContent?.includes('is available')",
      8_000,
      "truth-constrained business agent answer",
    );

    await evaluate(`(() => {
      const nav = document.querySelector('nav[aria-label="Asympta World mode"]');
      const button = [...(nav?.querySelectorAll('button') ?? [])]
        .find((item) => item.textContent?.trim() === 'Users');
      if (!button) throw new Error('Users mode button is unavailable.');
      button.click();
      return true;
    })()`);
    await waitFor(
      "!document.querySelector('[data-asympta-business-world=\"true\"]') && Boolean(document.querySelector('form.asympta-intent-composer'))",
      8_000,
      "return to Users world",
    );
    ...
    if (
      finalState.mode !== "users"
      || !finalState.userComposer
      || finalState.profileStored !== "Harbour Bakery"
      || finalState.productsStored !== 1
    ) {
      throw new Error(`Business/User mode persistence proof is incomplete: ${JSON.stringify(finalState)}`);
    }

    if (exceptions.length) {
      throw new Error(`Browser exceptions during Business mode: ${exceptions.join(" | ")}`);
    }

    console.log(
      "Business-mode browser smoke passed: Users stays default, Business imports real facts, agent communication is evidence-backed, and switching back preserves the existing world.",
    );
    socket.close();
  } catch (error) {
