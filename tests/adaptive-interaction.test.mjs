...
test("adaptive schema turns a TV screen-size gap into immediate useful choices", () => {
  const schema = createAdaptiveInteractionSchema({ intent: "I want to buy a television", missingFields: ["screen size", "budget", "brand preference"], locale: "en", interactionId: "tv-1", now: "2026-08-31T05:00:00.000Z" });
...
test("TV narrative gaps become four concrete option primitives in one schema", () => {
  const schema = createAdaptiveOptionPrimitiveSchema({ intent: "使用者想購買一台電視機", missingFields: ["使用者想購買一台電視機，需先釐清預算、尺寸、品牌偏好與配送地點等資訊。"], locale: "zh-Hant" });
...
test("reusable real-life primitive layer covers common choices without inventing unknown facts", () => {
  const scenarios = [
    ...
    ["テレビを買いたい", ["配送先", "ブランド"], ["delivery_location", "brand"]],
  ];
  ...
  const unknown = createAdaptiveOptionPrimitiveSchema({ intent: "Arrange a new kind of neighbourhood service", missingFields: ["locker compatibility protocol"], locale: "en" });
...
test("concert ticket gaps collapse into one high-information show choice", () => {
  const schema = createAdaptiveInteractionSchema({ intent: "使用者希望購買一張演唱會門票，預算彈性可接受高階", missingFields: ["演出、日期、地點、數量"], locale: "zh-Hant", interactionId: "concert-1", now: "2026-08-31T05:00:00.000Z" });
...
test("ticket planner does not repeat an explicit one-ticket quantity", () => {
  const fields = planAdaptiveMissingFields({ intent: "Buy one concert ticket", missingFields: ["artist", "date", "venue", "ticket count", "budget"] });
...
test("non-concert ticket requests are not forced into artist choices", () => {
  const fields = planAdaptiveMissingFields({ intent: "Buy one football match ticket", missingFields: ["match", "date", "venue"] });
...
test("unseen missing fields safely fall back to a generic text control instead of requiring new UI code", () => {
  const schema = createAdaptiveInteractionSchema({ intent: "Arrange a new kind of neighbourhood service", missingFields: ["locker compatibility protocol"], locale: "zh-Hant", interactionId: "unknown-1", now: "2026-08-31T05:00:00.000Z" });
...
test("clarification continuation contains only user-confirmed facts and preserves the original intent", () => {
  const intention = mergeAdaptiveClarifications({ intent: "Buy a TV", confirmations: [{ field: "screen size", value: "55-inch", label: "55″" }, { field: "budget", value: "balanced_value", label: "Best value / balanced" }], locale: "en" });
...
test("event confirmation tells the agent to search performances instead of asking date and venue fields", () => {
  const intention = mergeAdaptiveClarifications({ intent: "購買一張演唱會門票", confirmations: [{ field: "event intent", value: "hong_kong_popular", label: "香港近期熱門" }], locale: "zh-Hant" });
