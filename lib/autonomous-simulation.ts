...
function targetForAgent(world: WorldState, agent: LivingAgent): Destination {
  ...
  if (agent.status === "working") {
    const activeContract = world.contracts.find(
      (contract) =>
        ...
        (contract.leadAgentId === agent.id ||
          contract.collaboratorIds.includes(agent.id)),
    );
  ...
  const home = world.businesses.find((business) =>
    business.members.includes(agent.id),
  );
  ...
  if (home && wanderSeed < 0.64) {
    return {
      x:
        home.x +
        56 +
        stableNoise(world.tick, agent.id, "home-x") * (home.width - 112),
      y:
        home.y +
        68 +
        stableNoise(world.tick, agent.id, "home-y") * (home.height - 106),
      label: home.name,
      reason: "home base",
    };
  }
  ...
  return {
    x:
      MIN_X +
      stableNoise(world.tick, agent.id, "world-x") * (MAX_X - MIN_X),
    y:
      MIN_Y +
      stableNoise(world.tick, agent.id, "world-y") * (MAX_Y - MIN_Y),
    label: "open world",
    reason: "explore",
  };
}
...
function moveAgents(world: WorldState) {
  for (const raw of world.agents) {
    ...
    agent.x = Math.round(
      clamp(agent.x + (dx / distance) * step, MIN_X, MAX_X),
    );
    agent.y = Math.round(
      clamp(agent.y + (dy / distance) * step, MIN_Y, MAX_Y),
    );
  }
}
    ...
    if ((agent.energy ?? 70) < 38 && (agent.food ?? 0) > 0) {
      ...
      setThought(
        ...
        agent.status === "idle"
          ...
          : agent.status +
              " · energy " +
              String(Math.round(agent.energy ?? 0)),
        now,
      );
    }
  }
...
function maybeFoodTrade(world: WorldState, now: number) {
  const buyers = world.agents
    ...
    .filter(
      (agent) =>
        ...
        agent.balance >= 4 &&
        ((agent.energy ?? 80) < 66 ||
          stableNoise(world.tick, agent.id, "food-want") > 0.76),
    )
  ...
  const price = Math.max(
    ...
    Math.min(
      ...
      Math.round(
        3 +
          seller.priceSensitivity * 4 +
          stableNoise(world.tick, buyer.id, seller.id) * 2,
      ),
    ),
  );
  ...
  pushAutonomousEvent(world, {
    ...
    summary:
      "Low food stock triggered a direct " +
      String(price) +
      " credit trade between two agents.",
    origin: "native-agent",
...
function maybeResourceTrade(world: WorldState, now: number) {
  ...
  const price =
    5 +
    Math.floor(
      stableNoise(world.tick, buyer.id, seller.id, "resource-price") * 6,
    );
  if (buyer.balance < price) return false;
  ...
  pushAutonomousEvent(world, {
    ...
    summary:
      "Inventory pressure produced a voluntary " +
      String(price) +
      " credit exchange.",
    origin: "native-agent",
    actorIds: [buyer.id, seller.id],
...
function maybeSkillExchange(world: WorldState, now: number) {
  ...
  const teacher = nearestAgent(world, learner, (candidate) =>
    candidate.skills.some((skill) => !learner.skills.includes(skill)),
  );
  if (!teacher) return false;
  const skill = teacher.skills.find(
    (candidate) => !learner.skills.includes(candidate),
  );
  ...
  setThought(
    ...
    learned
      ? "Learned " + skill
      : skill + " practice " + String(progress) + "/3",
    now,
    teacher.id,
  );
  setThought(
  ...
  pushTrade(
    ...
    teacher.id,
    price,
    "Autonomous skill exchange: " + skill,
    now,
  );
  pushAutonomousEvent(world, {
...
function maybeEnquiry(world: WorldState, now: number) {
  const asker = world.agents
    ...
    .sort(
      (a, b) =>
        stableNoise(world.tick, b.id, "ask") -
        stableNoise(world.tick, a.id, "ask"),
    )[0];
  ...
  const text =
    topics[
      Math.floor(
        stableNoise(world.tick, asker.id, peer.id, "topic") * topics.length,
      )
    ];
  const message = {
  ...
  world.messages = [...world.messages, message].slice(-220);
...
export function advanceAutonomousSimulation(
  ...
  for (const agent of world.agents.map(living)) {
    hydrateAgent(agent, world, now);
  }
