...
import { Gauge, Handshake, Map, Play, Radio, Store, Users, X } from "lucide-react";
...
function saveSimulation(value: MassMarketSimulation) {
  try {
    ...
    // Memory-only mode remains valid.
  }
}
...
function emitReplayStep(trajectory: MarketTrajectory, stepIndex: number) {
  ...
  window.dispatchEvent(
    new CustomEvent("asympta:user-task-process", {
      detail: {
        label: progress >= 100 ? "Trajectory replay complete" : "Market trajectory · " + step.phase,
        detail: step.message,
        progress,
        tone:
          step.phase === "deal"
            ...
            : step.phase === "walkaway"
              ? "blocked"
              : ["bid", "counter", "concession"].includes(step.phase)
                ? "talking"
                : "planning",
      },
    }),
...
export function MarketTrajectoryRuntime() {
  ...
  const trajectories = useMemo(() => simulation?.trajectories ?? [], [simulation]);
  ...
  const enterTrajectory = useCallback((id: string) => {
    setSelectedId(id);
    setActiveStep(0);
    setEnteredId(id);
    setOpen(true);
  ...
  useEffect(() => {
    if (!entered) return;
    let index = 0;
    const first = window.setTimeout(() => emitReplayStep(entered, 0), 0);
  ...
  useEffect(() => {
    const onEnter = (event: Event) => {
      const id = (event as CustomEvent<{ trajectoryId?: string }>).detail?.trajectoryId;
      if (id) enterTrajectory(id);
    };
  ...
  useEffect(() => {
    ...
    const tools: MarketTool[] = [
      {
        ...
        description: "Read aggregate demand, bids, negotiations, deals, walkaways, clearing price and strategy wins from the headless 100,000-actor market simulation.",
        inputSchema: { type: "object", properties: {}, additionalProperties: false },
        annotations: { readOnlyHint: true, untrustedContentHint: false },
        execute: async () => JSON.stringify({ ok: Boolean(simulation), summary: simulation?.summary, strategies: MARKET_STRATEGIES }),
      },
      {
        name: "mass_market_list_trajectories",
        title: "List market negotiation trajectories",
        description: "List the representative trajectories that can be entered from the trajectory map.",
        inputSchema: { type: "object", properties: {}, additionalProperties: false },
        annotations: { readOnlyHint: true, untrustedContentHint: false },
      ...
      },
      {
        ...
        description: "Read demand broadcast, store hearing, bids, counters, concessions and the terminal deal or walkaway.",
      ...
      },
      {
        ...
        description: "Open the trajectory map and replay one negotiation sequence step by step.",
        inputSchema: { type: "object", properties: { trajectoryId: { type: "string" } }, required: ["trajectoryId"], additionalProperties: false },
        annotations: { readOnlyHint: false, untrustedContentHint: false },
        execute: async (input) => {
          const id = String(input.trajectoryId);
          if (!trajectories.some((item) => item.id === id)) return JSON.stringify({ ok: false, error: "Trajectory not found." });
      ...
      },
      {
        ...
        description: "Run another deterministic headless 100,000-actor simultaneous market batch and replace the trajectory sample.",
        inputSchema: { type: "object", properties: { seed: { type: "number" } }, additionalProperties: false },
        annotations: { readOnlyHint: false, untrustedContentHint: false },
        execute: async (input) => JSON.stringify({ ok: true, summary: runStress(typeof input.seed === "number" ? input.seed : Date.now()).summary }),
      },
    ];
    ...
    const modelContext = (document as unknown as {
      modelContext?: { registerTool: (tool: MarketTool, options?: { signal?: AbortSignal }) => Promise<void> | void };
    }).modelContext;
  ...
  return createPortal(
    <>
      <style>{`
        ...
        .trajectory-map-button{position:relative;display:grid;place-items:center;width:39px;height:39px;padding:0;border:1px solid rgba(112,124,115,.13);border-radius:50%;background:rgba(248,247,241,.86);color:#65746b;box-shadow:0 7px 22px rgba(49,60,53,.07);cursor:pointer}.trajectory-map-button svg{width:15px;height:15px}.trajectory-map-button b{position:absolute;right:-4px;top:-4px;min-width:19px;height:19px;padding:0 4px;border-radius:10px;background:#7184ad;color:white;font-family:var(--pixel-font);font-size:.26rem;line-height:19px}
        ...
        .trajectory-map-head{display:flex;align-items:flex-start;gap:8px}.trajectory-map-head>span{display:grid;gap:2px;min-width:0;flex:1}.trajectory-map-head strong{font-size:.66rem}.trajectory-map-head small{color:#7d8780;font-size:.38rem;line-height:1.35}.trajectory-close{display:grid;place-items:center;width:27px;height:27px;border:0;border-radius:50%;background:rgba(90,103,94,.06);color:#707a73;cursor:pointer}.trajectory-close svg{width:12px;height:12px}
        .trajectory-stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:5px}.trajectory-stat{display:grid;gap:3px;padding:7px;border-radius:10px;background:rgba(105,119,108,.055)}.trajectory-stat span{display:flex;align-items:center;gap:4px;color:#7b847e;font-family:var(--pixel-font);font-size:.27rem;text-transform:uppercase}.trajectory-stat span svg{width:9px;height:9px}.trajectory-stat b{font-size:.46rem}
        .trajectory-map-grid{display:grid;grid-template-columns:repeat(10,minmax(0,1fr));gap:4px;padding:8px;border:1px solid rgba(112,123,115,.1);border-radius:14px}.trajectory-node{aspect-ratio:1;padding:0;border:1px solid rgba(111,123,114,.13);border-radius:6px;background:rgba(102,117,106,.045);color:#758078;font-family:var(--pixel-font);font-size:.25rem;cursor:pointer}.trajectory-node[data-outcome="deal"]{background:rgba(103,143,117,.09);color:#5f7d66}.trajectory-node[data-outcome="walkaway"]{background:rgba(161,108,91,.07);color:#8a6960}.trajectory-node.is-selected{border-color:rgba(118,139,181,.48);box-shadow:0 0 0 2px rgba(118,139,181,.08)}
        .trajectory-selection,.trajectory-replay{display:grid;gap:7px;padding:9px;border-radius:12px;background:rgba(105,119,108,.045)}.trajectory-selection header{display:flex;align-items:center;gap:6px}.trajectory-selection header svg{width:12px;height:12px}.trajectory-selection p{margin:0;color:#707a73;font-size:.4rem;line-height:1.45}.trajectory-enter{display:flex;align-items:center;justify-content:center;gap:5px;min-height:32px;border:1px solid rgba(118,139,181,.2);border-radius:9px;background:rgba(118,139,181,.08);color:#59709d;font-family:var(--pixel-font);font-size:.3rem;cursor:pointer}.trajectory-enter svg{width:11px;height:11px}
        .trajectory-replay-summary{display:flex;flex-wrap:wrap;gap:5px}.trajectory-replay-summary span{padding:5px 6px;border-radius:8px;background:rgba(255,255,255,.24);font-family:var(--pixel-font);font-size:.29rem}.trajectory-step-list{display:grid;gap:4px}.trajectory-step{display:grid;grid-template-columns:64px minmax(0,1fr);gap:7px;padding:6px 7px;border-radius:9px;background:rgba(255,255,255,.16);opacity:.5}.trajectory-step.is-active{background:rgba(118,139,181,.08);opacity:1}.trajectory-step.is-past{opacity:.82}.trajectory-step b{color:#6c7fa7;font-family:var(--pixel-font);font-size:.28rem;text-transform:uppercase}.trajectory-step span{font-size:.39rem;line-height:1.38}.trajectory-rerun{border:0;background:transparent;color:#818983;font-family:var(--pixel-font);font-size:.27rem;cursor:pointer;text-decoration:underline}
        @media(max-width:620px){.trajectory-map-panel{left:9px;right:9px;top:max(58px,calc(env(safe-area-inset-top) + 50px));width:auto;max-height:calc(100svh - 150px)}.trajectory-stats{grid-template-columns:repeat(2,minmax(0,1fr))}.trajectory-map-grid{gap:3px;padding:6px}}
      `}</style>

      <div className="trajectory-map-control" onPointerDown={(event) => event.stopPropagation()}>
        <button type="button" className="trajectory-map-button" aria-label="Open 100 market trajectories" aria-expanded={open} onClick={() => setOpen((value) => !value)}>
          <Map aria-hidden="true" /><b>{trajectories.length || 100}</b>
        </button>
      </div>

      {open ? (
        <section className="trajectory-map-panel" aria-label="100 market negotiation trajectories" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}>
          <div className="trajectory-map-head">
            ...
            <span>
              ...
              <small>{entered ? "Replay the exact demand → bid → negotiation → outcome chain." : "100 representative causal paths sampled from one 100,000-actor simultaneous market tick."}</small>
            </span>
            <button type="button" className="trajectory-close" aria-label="Close trajectory map" onClick={() => { setOpen(false); setEnteredId(null); }}><X aria-hidden="true" /></button>
          </div>
            ...
            <>
              ...
              <div className="trajectory-map-grid" aria-label="Select a trajectory">
                {trajectories.map((trajectory) => (
                  <button type="button" key={trajectory.id} className={"trajectory-node" + (selectedId === trajectory.id ? " is-selected" : "")} data-outcome={trajectory.outcome} aria-label={trajectory.id + ", " + trajectory.title + ", " + trajectory.outcome} onClick={() => setSelectedId(trajectory.id)}>
              ...
              {selected ? (
                <section className="trajectory-selection">
                  ...
                  <p>{selected.buyer} · budget ₡{selected.budget.toFixed(0)} · {selected.competitors} competing stores · {selected.winnerStoreName} · {selected.winnerStrategy} · {selected.outcome}</p>
                  <button type="button" className="trajectory-enter" onClick={() => enterTrajectory(selected.id)}><Play aria-hidden="true" /> ENTER TRAJECTORY</button>
                </section>
              ) : null}
              <button type="button" className="trajectory-rerun" disabled={running} onClick={() => runStress(Date.now())}>{running ? "RUNNING 100,000-ACTOR STRESS…" : "RE-RUN 100,000-ACTOR STRESS"}</button>
            </>
          ) : (
            <section className="trajectory-replay">
              <div className="trajectory-replay-summary">
                <span>{entered.outcome.toUpperCase()}</span><span>{entered.winnerStrategy}</span><span>₡{entered.finalPrice?.toFixed(2)}</span><span>{entered.competitors} bidders</span>
              </div>
              <div className="trajectory-step-list">
                {entered.steps.map((step, index) => (
                  <div className={"trajectory-step" + (index === activeStep ? " is-active" : index < activeStep ? " is-past" : "")} key={String(index) + step.phase + step.actor}>
                    <b>{step.phase}</b><span>{step.message}</span>
                  </div>
                ))}
              </div>
              <button type="button" className="trajectory-enter" onClick={() => setEnteredId(null)}><Map aria-hidden="true" /> BACK TO 100 TRAJECTORIES</button>
            </section>
          )}
        </section>
      ) : null}
