...
type Approval = { id: string; title?: string; actionType?: string | null; taskId?: string | null };
type Ambient = { id: string; name: string; side: string; role: string; status: string; task: string };
type Snapshot = {
  foreground?: { tasks?: Task[]; agents?: Agent[]; pendingApprovals?: Approval[] };
  ambient?: Ambient[];
};
...
const META: Record<Locale, Record<string, string>> = {
  en: {
    "agent-user": "Personal intent agent", "agent-customer": "Customer advocate", "agent-business": "Business coordinator", "agent-supplier": "Supplier agent",
    "agent-operations": "Operations planner", "agent-finance": "Finance controller", "agent-logistics": "Logistics dispatcher", "agent-support": "Service recovery agent",
    "agent-quality": "Quality verifier", "agent-market": "Market intelligence agent",
  },
  "zh-Hant": {
    "agent-user": "個人需求代理", "agent-customer": "客戶需求代理", "agent-business": "商業協調代理", "agent-supplier": "供應代理",
    "agent-operations": "營運規劃代理", "agent-finance": "財務控制代理", "agent-logistics": "物流調度代理", "agent-support": "服務復原代理",
    "agent-quality": "品質驗證代理", "agent-market": "市場情報代理",
  },
  ja: {
    "agent-user": "個人意図エージェント", "agent-customer": "顧客担当エージェント", "agent-business": "ビジネス調整エージェント", "agent-supplier": "サプライヤーエージェント",
    "agent-operations": "オペレーション計画エージェント", "agent-finance": "財務管理エージェント", "agent-logistics": "物流配車エージェント", "agent-support": "サービス復旧エージェント",
    "agent-quality": "品質検証エージェント", "agent-market": "市場情報エージェント",
  },
};
...
const TOOLS: Record<Locale, Record<string, string>> = {
  en: { asympta_observe_living_city: "Observe living city", asympta_list_workflows: "List workflows", asympta_follow_agent: "Follow agent", asympta_request_workflow: "Request workflow", asympta_request_external_action: "Request external action" },
  "zh-Hant": { asympta_observe_living_city: "檢視協作城市", asympta_list_workflows: "列出工作流程", asympta_follow_agent: "跟隨代理", asympta_request_workflow: "請求工作流程", asympta_request_external_action: "請求外部動作" },
  ja: { asympta_observe_living_city: "協調都市を観察", asympta_list_workflows: "ワークフロー一覧", asympta_follow_agent: "エージェントを追従", asympta_request_workflow: "ワークフローを要求", asympta_request_external_action: "外部アクションを要求" },
};
  ...
  useEffect(() => {
    const sync = () => {
      ...
      for (const agent of agents) {
        ...
        const role = META[lang][agent.id] ?? agent.role;
        const dialogue = approval ? copy.waitingApproval : task ? localTask(lang, task) : role;
        setData(marker.querySelector(".animal-map-marker__dialogue"), "asymptaLocaleText", dialogue);
        marker.title = `${agent.name} · ${role}`;
      }
      ...
      if (pending && approvalCard) {
        ...
        const action = pending.actionType ? ACTIONS[lang][pending.actionType] : "";
