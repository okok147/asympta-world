...
import { Camera, Check, PackageSearch, Sparkles, Store, Wrench, X } from "lucide-react";
...
function loadState(): FounderState {
  try {
    ...
    if (!raw) return { version: 1, projects: [], places: [], lastEvaluationAt: 0 };
    const parsed = JSON.parse(raw) as FounderState;
    return parsed.version === 1 && Array.isArray(parsed.projects) && Array.isArray(parsed.places)
      ? parsed
      : { version: 1, projects: [], places: [], lastEvaluationAt: 0 };
  } catch {
    return { version: 1, projects: [], places: [], lastEvaluationAt: 0 };
  }
}

function saveState(state: FounderState) {
  try {
    localStorage.setItem(FOUNDER_KEY, JSON.stringify(state));
  } catch {
    // Memory fallback.
  }
}
...
function captureReferenceScreenshot(project: StoreProject) {
  ...
  context.font = "12px system-ui";
  const preview = project.catalog.slice(0, 4);
...
function choosePosition(index: number) {
  const positions = [
    [305, 232], [930, 255], [650, 505], [300, 545], [990, 500], [500, 245],
  ] as const;
...
export function CommunityStoreFounderRuntime() {
  const stateRef = useRef<FounderState>({ version: 1, projects: [], places: [], lastEvaluationAt: 0 });
  ...
  const startProject = useCallback((template?: Template, triggerNeed?: string) => {
    ...
    if (current.projects.some((project) => project.phase !== "open")) return current.projects.find((project) => project.phase !== "open");
    ...
    const next = { ...current, projects: [project, ...current.projects].slice(0, 8), lastEvaluationAt: now };
    commit(next);
    setProjectStatus(phaseLabel(project.phase) + " · " + project.name);
    return project;
  }, [commit]);
  ...
  const publishProject = useCallback((project: StoreProject, now: number) => {
    ...
    const templateKey = TEMPLATES.find((template) => template.name === project.name)?.key ?? project.id;
    const place: FoundedPlace = {
      id: "founded-" + templateKey,
      name: project.name,
    ...
    commit(next);
    setProjectStatus("正式開張 · " + place.name);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      const current = stateRef.current;
      const now = Date.now();
      let active = current.projects.find((project) => project.phase !== "open");
      if (!active) {
        const firstAllowed = current.lastEvaluationAt === 0 ? FIRST_AUTONOMOUS_START_MS : NEXT_PROJECT_COOLDOWN_MS;
        if (now - current.lastEvaluationAt >= firstAllowed && current.places.length < MAX_FOUNDED_PLACES) {
          active = startProject();
        }
        return;
      }
      ...
      const updated: StoreProject = {
        ...
        phaseStartedAt: now,
        updatedAt: now,
        screenshot: next === "catalog" && !active.screenshot ? captureReferenceScreenshot(active) : active.screenshot,
      };
      const nextState = {
        ...current,
        projects: current.projects.map((project) => project.id === active?.id ? updated : project),
      };
      commit(nextState);
      setProjectStatus(phaseLabel(next) + " · " + updated.name);
    }, 1000);
  ...
  const executeFoundedAction = useCallback((place: FoundedPlace, action: "inspect" | "buy_product" | "book_service" | "inquire", itemId?: string) => {
    ...
    if (!item) return { ok: false, summary: "No suitable item is available." };
    if (item.available <= 0) return { ok: false, summary: "No capacity is available." };
    item.available -= 1;
  ...
  useEffect(() => {
    ...
    const tools: RegistryTool[] = [
      ...
      },
      {
        ...
        description: "Start a demand-driven store/service founding project. The project must gather evidence, catalog information and review before opening.",
        inputSchema: {
          type: "object",
          properties: { template: { type: "string", enum: TEMPLATES.map((template) => template.key) }, triggerNeed: { type: "string" } },
      ...
      },
      {
        ...
        description: "Attach a real browser screenshot data URL/reference when available, or generate a local reference snapshot from the proposed store information.",
        inputSchema: {
          type: "object",
          properties: { projectId: { type: "string" }, screenshot: { type: "string", maxLength: 1200000 } },
          required: ["projectId"],
          additionalProperties: false,
        },
        annotations: { readOnlyHint: false, untrustedContentHint: true },
        execute: async (input) => {
          ...
          const updated = { ...project, screenshot, phase: "catalog" as const, phaseStartedAt: Date.now(), updatedAt: Date.now() };
      ...
      },
      {
        ...
        description: "Write the product and service list extracted from research or screenshots into the founding project before review.",
        inputSchema: {
          type: "object",
          properties: {
            ...
            items: {
              ...
              items: {
                type: "object",
                properties: {
                  id: { type: "string" }, name: { type: "string" }, type: { type: "string", enum: ["product", "service"] },
                  price: { type: "number", minimum: 0, maximum: 10000 }, capacity: { type: "number", minimum: 1, maximum: 100 },
        ...
        execute: async (input) => {
          ...
          const catalog = (input.items as Array<Record<string, unknown>>).map((item, index) => ({
            ...
            available: Math.max(1, Math.floor(Number(item.capacity) || 1)),
            tags: Array.isArray(item.tags) ? item.tags.map(String).slice(0, 8) : [],
          }));
          const updated = { ...project, catalog, phase: "review" as const, phaseStartedAt: Date.now(), updatedAt: Date.now() };
          commit({ ...current, projects: current.projects.map((candidate) => candidate.id === project.id ? updated : candidate) });
      ...
      },
      {
        ...
        execute: async (input) => {
          ...
          const place = stateRef.current.places.find((candidate) => candidate.name === project.name);
          return JSON.stringify({ ok: Boolean(place), place });
        },
      },
      {
        name: "community_search_founded_stores",
        title: "Search community-founded stores and services",
        description: "Search stores that were created by the community founding workflow.",
      ...
      },
      {
        ...
        inputSchema: {
          type: "object",
          properties: {
            placeId: { type: "string" }, action: { type: "string", enum: ["inspect", "buy_product", "book_service", "inquire"] }, itemId: { type: "string" },
          },
          required: ["placeId", "action"], additionalProperties: false,
        },
        annotations: { readOnlyHint: false, untrustedContentHint: true },
        execute: async (input) => {
          ...
          const action = ["inspect", "buy_product", "book_service", "inquire"].includes(String(input.action)) ? String(input.action) as "inspect" | "buy_product" | "book_service" | "inquire" : "inspect";
    ...
    const fallback = window as unknown as {
      __ASYMPTA_FOUNDING_WEBMCP__?: { tools: RegistryTool[]; invoke: (name: string, input?: Record<string, unknown>) => Promise<unknown> };
    };
    ...
    return () => {
      controller.abort();
      delete fallback.__ASYMPTA_FOUNDING_WEBMCP__;
    };
  ...
  return (
    <>
      <style>{`
        .founded-community-place { color:#6c8075; }
        .founded-community-place::before { content:""; position:absolute; inset:4px 18px 19px; border-radius:50%; pointer-events:none; opacity:0; }
        .founded-community-place.is-celebrating { opacity:1!important; }
        .founded-community-place.is-celebrating::before {
          opacity:1;
          box-shadow:0 0 14px rgba(174,151,88,.32),0 0 34px rgba(113,149,121,.26),0 0 58px rgba(118,139,181,.18);
          animation:community-opening-glow 1.45s ease-in-out 6;
        }
        .founded-celebration { position:absolute; left:50%; top:26px; width:4px; height:4px; border-radius:50%; background:#d3b76b; box-shadow:-22px -13px #91ad9a,20px -16px #9aa9c7,-28px 8px #c69b77,27px 9px #b39bbd,0 -27px #d6c783; pointer-events:none; animation:community-opening-spark 1.4s ease-out 6; }
        @keyframes community-opening-glow { 0%,100%{transform:scale(.88);opacity:.34}50%{transform:scale(1.28);opacity:1} }
        @keyframes community-opening-spark { 0%{transform:translate(-50%,-50%) scale(.55);opacity:0}32%{opacity:1}100%{transform:translate(-50%,-50%) scale(1.45);opacity:0} }
        .founding-status-chip { position:absolute; z-index:91; left:50%; top:max(15px,env(safe-area-inset-top)); max-width:280px; padding:6px 8px; transform:translateX(-50%); border:1px solid rgba(111,124,115,.15); border-radius:10px; background:rgba(248,247,241,.84); color:#727c75; font-family:var(--pixel-font); font-size:.3rem; pointer-events:none; backdrop-filter:blur(10px); opacity:.8; }
        .founded-evidence { display:block; width:100%; aspect-ratio:12/7; margin-top:8px; object-fit:cover; border:1px solid rgba(112,121,114,.12); border-radius:9px; opacity:.78; }
        @media(prefers-reduced-motion:reduce){.founded-community-place.is-celebrating::before,.founded-celebration{animation:none!important}}
      `}</style>

      {createPortal(
        <>
          {state.places.map((place) => {
            const celebrating = place.celebratingUntil > Date.now();
            return (
              <button
                type="button"
                className={"community-place founded-community-place" + (celebrating ? " is-celebrating" : "")}
                key={place.id}
                style={{ left: place.x, top: place.y }}
                aria-label={place.name + ", community-founded store"}
                onPointerDown={(event) => event.stopPropagation()}
                onClick={() => setSelectedId(place.id)}
              >
                <svg viewBox="0 0 50 36" aria-hidden="true">{linePaths(place.seed).map((path, index) => <path d={path} key={index} />)}</svg>
                <span className="community-place-name">{place.name}</span>
                {celebrating ? <span className="founded-celebration" aria-hidden="true" /> : null}
              </button>
            );
          })}
        </>,
        worldPlane,
        "community-founded-places",
      )}

      {active || projectStatus ? createPortal(
        <span className="founding-status-chip">{active ? phaseLabel(active.phase) + " · " + active.name : projectStatus}</span>,
        viewport,
        "community-founding-status",
      ) : null}

      {selected ? createPortal(
        <section className="community-inspector" style={{ left: 14, top: 62 }} aria-label={selected.name + " founded store information"}>
          <header>
            <span><small>community founded</small><strong>{selected.name}</strong></span>
            <button type="button" className="community-close" aria-label="Close founded store" onClick={() => setSelectedId(null)}><X aria-hidden="true" /></button>
          </header>
          <div className="community-stats">
            <span className="community-stat"><Store aria-hidden="true" />rep {selected.reputation}</span>
            <span className="community-stat"><PackageSearch aria-hidden="true" />{selected.catalog.reduce((sum, item) => sum + item.available, 0)} available</span>
            <span className="community-stat"><Camera aria-hidden="true" />evidence</span>
          </div>
          <span className="community-inspector-label">{selected.summary}</span>
          {selected.screenshot ? <img className="founded-evidence" src={selected.screenshot} alt={selected.name + " research screenshot evidence"} /> : null}
          <div className="community-offerings">
            {selected.catalog.map((item) => (
              <div className="community-offering" key={item.id}><span>{item.name}</span><small>{item.available}/{item.capacity} · ₡{item.price}</small></div>
            ))}
          </div>
          <div className="community-actions">
            <button type="button" className="community-action" onClick={() => executeFoundedAction(selected, "inspect")}>Inspect</button>
            <button type="button" className="community-action" onClick={() => executeFoundedAction(selected, "buy_product")}>Buy</button>
            <button type="button" className="community-action" onClick={() => executeFoundedAction(selected, "book_service")}>Book</button>
            <button type="button" className="community-action" onClick={() => executeFoundedAction(selected, "inquire")}>Inquire</button>
          </div>
        </section>,
        viewport,
        "founded-store-inspector",
      ) : null}
    </>
  );
}
