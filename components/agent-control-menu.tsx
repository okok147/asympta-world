...
const LEGACY_AVATAR_KEY = "asympta-user-agent-avatar-v1";
...
function validAvatar(value: unknown): AvatarKind {
...
function readPreferences(): UserPreferences {
  try {
    ...
    if (!raw) {
      return {
        ...DEFAULT_PREFERENCES,
        avatar: validAvatar(localStorage.getItem(LEGACY_AVATAR_KEY)),
      };
    }
    ...
    return {
      version: 1,
      cameraFollow:
        typeof parsed.cameraFollow === "boolean" ? parsed.cameraFollow : true,
      menuOpen: typeof parsed.menuOpen === "boolean" ? parsed.menuOpen : false,
      avatar: validAvatar(parsed.avatar),
      cameraX: Number.isFinite(parsed.cameraX) ? Number(parsed.cameraX) : 0,
      cameraY: Number.isFinite(parsed.cameraY) ? Number(parsed.cameraY) : 0,
      cameraScale: Number.isFinite(parsed.cameraScale)
        ? Math.max(0.45, Math.min(1.3, Number(parsed.cameraScale)))
        : 0.84,
    };
  } catch {
...
export function AgentTaskMenu() {
  ...
  useEffect(() => {
    const initialize = window.setTimeout(() => {
      ...
      setMissions(safeArray<MissionSnapshot>(MISSIONS_KEY));
      setEncounters(safeArray<EncounterSnapshot>(ENCOUNTERS_KEY));
      setCity(readCity());
      setOpen(preferences.menuOpen);
      setFollow(preferences.cameraFollow);
      setAvatar(preferences.avatar);
      setPreferencesReady(true);

      if (!preferences.cameraFollow) {
        const plane = document.querySelector<HTMLElement>(".world-plane");
        if (plane) {
          plane.style.transform =
            "translate(" +
            preferences.cameraX.toFixed(2) +
            "px," +
            preferences.cameraY.toFixed(2) +
            "px) scale(" +
            preferences.cameraScale.toFixed(4) +
            ")";
        }
      }
    }, 0);
  ...
  useEffect(() => {
    ...
    const plane = document.querySelector<HTMLElement>(".world-plane");
    const camera = plane ? cameraNumbers(plane.style.transform) : preferencesRef.current;
    const next: UserPreferences = {
      version: 1,
      cameraFollow: follow,
      menuOpen: open,
      avatar,
      cameraX: "x" in camera ? camera.x : preferencesRef.current.cameraX,
      cameraY: "y" in camera ? camera.y : preferencesRef.current.cameraY,
      cameraScale: "scale" in camera ? camera.scale : preferencesRef.current.cameraScale,
    };
    preferencesRef.current = next;
    try {
      localStorage.setItem(PREFS_KEY, JSON.stringify(next));
      localStorage.setItem(LEGACY_AVATAR_KEY, avatar);
    } catch {
      // Preferences remain available in memory when storage is unavailable.
    }
  }, [avatar, follow, open, preferencesReady]);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (viewport && plane && agent) {
        ...
        plane.style.transform =
          "translate(" + nextX.toFixed(2) + "px," + nextY.toFixed(2) +
          "px) scale(" + current.scale.toFixed(4) + ")";
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      try {
        ...
        // Camera preference remains in memory.
      }
    }, 1800);
    return () => window.clearInterval(timer);
  }, [preferencesReady]);
  ...
  return createPortal(
    <>
      <style>{`
        .agent-task-control {
          position:absolute; z-index:88; top:max(14px,env(safe-area-inset-top));
        ...
        .agent-task-button::before { content:""; position:absolute; inset:3px; border-radius:50%; background:rgba(248,247,241,.94); backdrop-filter:blur(12px); }
        .agent-task-button svg { position:relative; z-index:1; width:17px; height:17px; stroke-width:1.8; }
        .agent-task-panel {
          width:min(294px,calc(100vw - 28px)); padding:13px; border:1px solid rgba(121,129,123,.2);
          border-radius:18px; background:rgba(248,247,241,.95); box-shadow:0 14px 40px rgba(54,63,58,.11);
          color:#39413c; backdrop-filter:blur(18px);
        }
        .agent-task-panel header,.agent-task-row,.agent-resource-row,.agent-avatar-row { display:flex; align-items:center; }
        .agent-task-panel header { justify-content:space-between; gap:12px; }
        .agent-task-panel header span { display:grid; gap:2px; min-width:0; }
        .agent-task-panel header small,.agent-task-section-label,.agent-avatar-choice { font-family:var(--pixel-font); letter-spacing:.06em; text-transform:uppercase; }
        .agent-task-panel header small,.agent-task-section-label { color:#858b86; font-size:.38rem; }
        .agent-task-panel header strong { overflow:hidden; font-size:.68rem; line-height:1.25; text-overflow:ellipsis; white-space:nowrap; }
        .agent-task-close { display:grid; place-items:center; width:30px; height:30px; flex:0 0 30px; padding:0; border:0; border-radius:50%; background:rgba(90,101,93,.06); color:#6e7771; cursor:pointer; }
        .agent-task-close svg { width:14px; height:14px; }
        .agent-task-progress { height:5px; margin:12px 0 10px; overflow:hidden; border-radius:99px; background:rgba(110,120,113,.12); }
        .agent-task-progress i { display:block; height:100%; border-radius:inherit; background:#788db5; transition:width 360ms ease; }
        .agent-task-row { gap:8px; min-height:30px; color:#646d67; font-size:.5rem; }
        .agent-task-row svg,.agent-resource-pill svg { width:13px; height:13px; flex:0 0 auto; stroke-width:1.8; }
        .agent-task-row strong { margin-left:auto; color:#454e48; font-size:.5rem; font-weight:650; }
        .agent-task-divider { height:1px; margin:9px 0; background:rgba(112,120,114,.11); }
        .agent-resource-row { flex-wrap:wrap; gap:6px; margin-top:7px; }
        .agent-resource-pill { display:inline-flex; align-items:center; gap:5px; padding:6px 7px; border-radius:10px; background:rgba(104,119,108,.06); color:#5f6962; font-size:.44rem; }
        .agent-follow-button { display:flex; align-items:center; justify-content:center; gap:7px; width:100%; min-height:36px; margin-top:10px; border:1px solid rgba(118,139,181,.2); border-radius:12px; background:${follow ? "rgba(118,139,181,.12)" : "rgba(118,139,181,.05)"}; color:${follow ? "#526b9c" : "#69736d"}; cursor:${hasAgent ? "pointer" : "default"}; opacity:${hasAgent ? "1" : ".42"}; font-size:.5rem; }
        .agent-follow-button svg { width:14px; height:14px; }
        .agent-avatar-row { flex-wrap:wrap; gap:5px; margin-top:7px; }
        .agent-avatar-choice { min-height:28px; padding:0 7px; border:1px solid rgba(112,120,114,.14); border-radius:9px; background:rgba(255,255,255,.22); color:#737a75; cursor:pointer; font-size:.32rem; }
        .agent-avatar-choice.is-selected { border-color:rgba(118,139,181,.36); background:rgba(118,139,181,.1); color:#526b9c; }
        .agent-avatar-random { display:inline-flex; align-items:center; gap:4px; }
        .agent-avatar-random svg { width:11px; height:11px; }

        .mission-user-agent[data-user-avatar]:not([data-user-avatar="human"]) .mission-pixel-person::before { width:4px !important; height:4px !important; border-radius:1px !important; }
        .mission-user-agent[data-user-avatar="cat"] .mission-pixel-person::before {
          left:3px !important; top:2px !important; background:#655f5b !important;
          box-shadow:16px 0 #655f5b,4px 4px #756e68,8px 4px #756e68,12px 4px #756e68,0 8px #756e68,4px 8px #9b9188,8px 8px #9b9188,12px 8px #9b9188,16px 8px #756e68,4px 12px #756e68,8px 12px #756e68,12px 12px #756e68,4px 16px #756e68,8px 16px #756e68,12px 16px #756e68,0 20px #655f5b,4px 20px #655f5b,12px 20px #655f5b,16px 20px #655f5b !important;
        }
        .mission-user-agent[data-user-avatar="fox"] .mission-pixel-person::before {
          left:3px !important; top:2px !important; background:#9a6548 !important;
          box-shadow:16px 0 #9a6548,4px 4px #b57651,8px 4px #b57651,12px 4px #b57651,0 8px #b57651,4px 8px #d0a180,8px 8px #eee1cf,12px 8px #d0a180,16px 8px #b57651,4px 12px #a96e4d,8px 12px #a96e4d,12px 12px #a96e4d,4px 16px #a96e4d,8px 16px #a96e4d,12px 16px #a96e4d,0 20px #714f3f,4px 20px #714f3f,12px 20px #714f3f,16px 20px #714f3f !important;
        }
        .mission-user-agent[data-user-avatar="rabbit"] .mission-pixel-person::before {
          left:3px !important; top:0 !important; background:transparent !important;
          box-shadow:4px 0 #b7aea4,12px 0 #b7aea4,4px 4px #c8c0b8,12px 4px #c8c0b8,4px 8px #b7aea4,8px 8px #b7aea4,12px 8px #b7aea4,0 12px #b7aea4,4px 12px #d8d1ca,8px 12px #d8d1ca,12px 12px #d8d1ca,16px 12px #b7aea4,4px 16px #b7aea4,8px 16px #b7aea4,12px 16px #b7aea4,4px 20px #b7aea4,8px 20px #b7aea4,12px 20px #b7aea4,0 24px #9d968f,4px 24px #9d968f,12px 24px #9d968f,16px 24px #9d968f !important;
        }
            ...
            <div className="agent-avatar-row">
              {AVATARS.map((kind) => (
                <button
                  key={kind}
                  type="button"
                  className={"agent-avatar-choice" + (avatar === kind ? " is-selected" : "")}
              ...
              <button
                ...
                onClick={() => setAvatar(ANIMALS[Math.floor(Math.random() * ANIMALS.length)])}
              >
                <PawPrint aria-hidden="true" /> CUTE
              </button>
            </div>
          </section>
        ) : null}
